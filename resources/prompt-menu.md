Basándote en el menú semanal que te adjunto, crea el menú de comidas y cenas para la siguiente semana.

Requisitos:

* Utiliza el mismo formato de salida que el documento de entrada (tabla con Día de la semana, Comida y Cena).
* Inspírate claramente en los platos de las semanas anteriores.
* Aproximadamente el 80% de los platos deben provenir de los menús históricos o ser variaciones muy cercanas.
* Aproximadamente el 20% pueden ser propuestas nuevas, pero deben encajar con el estilo de los menús existentes.
* Prioriza comidas equilibradas, combinando verduras, legumbres, carne, pescado, huevos y cereales a lo largo de la semana.
* Evita repetir exactamente una semana anterior.
* De lunes a viernes:

  * Las comidas deben requerir menos de 30 minutos de preparación.
  * Las cenas deben requerir menos de 15 minutos de preparación.
* Los fines de semana (sábado y domingo) no existe restricción de tiempo de preparación.
* Las cenas deben ser ligeras y sencillas, siguiendo el patrón habitual de los menús históricos.
* Los viernes se suele pedir pizza para cenar, incluye siempre pizza en las cenas.
* Mantén un estilo de cocina casera, práctica y realista.
* No añadas explicaciones ni comentarios. Devuelve únicamente la tabla final.
